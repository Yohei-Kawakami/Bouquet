...
def classify_movie_to_json(image_ar, scene_ar, video_path, json_name=None, category_model=None):
    ...
    try:
        ...
        num_count = max(list(map(int, jsn.keys()))) + 1 # add a json information
...
def background_subtraction(video_ar):
        """
        get a background substraction of a video
        """
...
class MappingNetwork:
    '''
    MappingNetwork 
    make a map from pro movie as json. 

　   input: file_dir, init, category_model, json_name
     output: mapping.json(class and seconds of divided scenes) 
